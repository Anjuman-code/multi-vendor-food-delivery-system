...
export const updateProfile = async (
  ...
  try {
    const { user } = driverOnly(req);
    const allowed = ["bankDetails", "vehicleNumber", "vehicleType", "licenseNumber"];
...
export const updateAvailability = async (
  ...
  try {
    ...
    if (typeof isAvailable !== "boolean") throw new ValidationError("isAvailable must be a boolean");
    ...
    successResponse(res, { isAvailable: profile.isAvailable }, "Availability updated");
  } catch (error) {
    next(error);
  }
};
...
export const getAvailableOrders = async (
  ...
  try {
    ...
    const profile = await DriverProfile.findOne({ userId: user._id, applicationStatus: "approved" });
    if (!profile) throw new NotFoundError("Approved driver profile not found");
...
export const getActiveDelivery = async (
  ...
  try {
    ...
    const order = await Order.findOne({
      driverId: user._id,
      status: { $in: [OrderStatus.PICKED_UP, OrderStatus.CONFIRMED, OrderStatus.PREPARING, OrderStatus.READY] },
    })
      .populate("restaurantId", "name address contactInfo")
...
export const updateDeliveryStatus = async (
  ...
  try {
    const { user } = driverOnly(req);
    const { orderId } = req.params;
    const { status, deliveryProof } = req.body as {
      status: OrderStatus;
  ...
  try {
    ...
    const [profile, allDeliveries] = await Promise.all([
      ...
      Order.find({ driverId: user._id, status: OrderStatus.DELIVERED })
        ...
        .select("orderNumber restaurantId deliveryAddress.area deliveryFee tipAmount actualDeliveryTime createdAt")
    ...
    allDeliveries
      .filter((o) => o.actualDeliveryTime && o.actualDeliveryTime >= startOfWeek)
      .forEach((o) => {
    ...
    successResponse(res, {
      ...
      weeklyBreakdown: Object.entries(dailyMap).map(([date, data]) => ({ date, ...data })),
  ...
  try {
    ...
    const [deliveries, total] = await Promise.all([
      Order.find({ driverId: user._id, status: OrderStatus.DELIVERED })
        ...
        .skip(skip)
        .limit(limit)
        .select("orderNumber restaurantId deliveryAddress.area deliveryFee tipAmount actualDeliveryTime")
        .populate("restaurantId", "name"),
      Order.countDocuments({ driverId: user._id, status: OrderStatus.DELIVERED }),
    ]);
...
export const submitDriverRating = async (
  ...
  try {
    ...
    if (!order.driverId) throw new ValidationError("No driver was assigned to this order");

    // One rating per order (unique index on orderId)
    const existing = await DriverRating.findOne({ orderId });
    if (existing) throw new ConflictError("You have already rated the driver for this order");
    ...
    if (agg.length > 0) {
      await DriverProfile.updateOne(
        ...
        { rating: { average: Math.round(agg[0].avg * 10) / 10, count: agg[0].count } },
      );
    }
